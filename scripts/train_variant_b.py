...
def main():
    ...
    train_ds = DRGradingDataset(
        os.path.join(SPLITS_DIR, "train.csv"), image_size=IMAGE_SIZE, transform=get_training_augmentations(IMAGE_SIZE)
    )
    val_ds = DRGradingDataset(os.path.join(SPLITS_DIR, "val.csv"), image_size=IMAGE_SIZE)
    test_ds = DRGradingDataset(os.path.join(SPLITS_DIR, "test.csv"), image_size=IMAGE_SIZE)
